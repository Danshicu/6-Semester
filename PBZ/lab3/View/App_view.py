...
class App(Tk):
    ...
    def __init__(self):
        super().__init__()
        main_menu = Menu(self, tearoff=1)
        main_menu.add_command(label="Добавление", command=self.creation_window, background='green')
        ...
        self.config(menu=main_menu, bg='white')
        ...
        self.notebook = ttk.Notebook(self, width=1000, height=950)
        vocabulary_frame = Frame(self.notebook, bd=4, padx=20, pady=10)
        obj_prop_frame = Frame(self.notebook, bd=4, padx=20, pady=10)
        subj_pred_obj_frame = Frame(self.notebook, bd=4, padx=20, pady=10)
        individuals_tree_frame = Frame(self.notebook, bd=4, padx=20, pady=10)
        ...
        self.vocabularyTree = ttk.Treeview(vocabulary_frame, show='tree', height=45)
        self.vocabularyTree.column('#0', stretch=YES, minwidth=0, width=950)
        ...
        self.objPropTree = ttk.Treeview(obj_prop_frame, show='tree', height=45)
        self.objPropTree.column('#0', stretch=YES, minwidth=0, width=950)
        ...
        self.subjPredObjTree.column('#1', stretch=NO, minwidth=100, width=300)
        self.subjPredObjTree.column('#2', stretch=NO, minwidth=100, width=300)
        self.subjPredObjTree.column('#3', stretch=NO, minwidth=100, width=300)
        self.subjPredObjTree.grid(row=0, column=0, sticky='nsew')

        self.individualsTree = ttk.Treeview(
            individuals_tree_frame, columns="Individuals", selectmode='browse', height=45)
        ...
        self.individualsTree.column('#1', stretch=NO, minwidth=100, width=950)
    ...
    def open_individuals_attributes(self, event):
        ...
        name_label = Label(ind_frame, text="Name", width=20)
        name_value = Label(ind_frame, text=item_name, width=20)

        class_label = Label(ind_frame, text="Class", width=20)
        class_value = Label(ind_frame, text=class_name, width=40)

        prop_label = Label(ind_frame, text="Properties", width=20)
        ...
        for c in self.class_dictionary:
            for cl in self.class_dictionary:
                if cl.subClasses.__contains__(c.name):
                    if root_classes.__contains__(c):
    ...
    def load_ontology(self):
        filename = askopenfilename(filetypes=(("rdf file", "*.rdf"), ("owl file", "*.owl")), defaultextension=("rdf file", "*.rdf"))
        if filename is None:
            return
    ...
    def load_classes(self):
        for s, p, o in self.graph:
            ...
            if o == OWL.Class and p == RDF.type:
                ...
                self.hint_class.append(oc.name)
                # print(self.hint_class)
            ...
            if o == OWL.ObjectProperty and p == RDF.type:
                for sub, pre, obj in self.graph:
                    if pre.__repr__() == s_str:
                        ...
                        self.hint_subject.append(ob.subject)
                        self.hint_object.append(ob.object)
        # print(f'subjects: {self.hint_subject}')
        # print(f'objects: {self.hint_object}')
    ...
    def save_ontology(self):
        file = asksaveasfile(filetypes=(("owl file", "*.owl"),("rdf file", "*.rdf")), defaultextension=("rdf file", "*.rdf"))
        if file is None:
            return
